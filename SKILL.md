...
### 步骤4.4：识典古籍原文链接验证（短引链接门禁）

确定最终回答要保留的每条史料原文短引后，尽量为该短引生成识典古籍原文章节链接：
...
- 如果返回 `not_found` 或 `invalid`，可以把识典检索页作为内部复核线索，但不得在正文中称为“原文链接”；可简短写“未能确认可靠识典原文链接”，或直接省略。
...
8. **识典原文链接**（短引能验证时附链接；不能验证时不猜）⭐
9. **古地名今地说明与左图右史链接**（首次出现处自然括注现代行政区划；能确认同代一级区划时附地图链接）⭐
10. **分析解释**（**仅基于查询到的事实**）
...
- [ ] 是否避免把原文短引混在正文叙述段落里？
- [ ] 是否对最终保留的原文短引运行过识典链接验证；只在 `resolved` 时写 `[识典原文](...)`？
- [ ] 是否把所有年号纪年保留原文纪年并换算为公元纪年？
- [ ] 是否避免只写公元年、遗漏史料年号？
- [ ] 是否先列出草稿中所有保留的古地名，并逐一查询/确认今地（如适用）？
- [ ] 是否把每个保留古地名的首次出现处都用自然括注标注现代行政区划，查不到时写明“今地未能确认”？
- [ ] 是否先用辞典/原文确认地点的同时代一级行政区，再只在 `history_map_link.py` 返回 `resolved` 时附左图右史链接？
