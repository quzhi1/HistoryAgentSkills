...
| 书目链接索引 | `data/source_book_index.json` | 识典书页 + cnkgraph 书目/API 链接映射，供识典原书名核对 |
...
│   └── source_book_index.json      # 识典书页与 cnkgraph 书目/API 链接索引
│
...
- ✅ 识典原文链接使用 `scripts/shidian_link.py`；脚本会读取 `data/source_book_index.json` 核对识典书页与 cnkgraph 书名。返回 `resolved` 且 `matched_source` 原书名核对通过，才能写 `[识典原文](...)`；若候选是后代转引或异书，或返回 `not_found` / `invalid` / 查询失败，必须重新收窄 `--quote` / `--keyword`、改用原书别名或篇名/卷名，优先换成可 `resolved` 的同义证据短引；仍不能确认时要在答案中明示“识典原文链接二次验证未通过”
- ✅ 书目链接索引用 `scripts/update_source_book_index.py` 刷新；识典来自 sitemap 书页，cnkgraph 来自 `GET /api/Book` 及 `GET /api/Book/{部}/{类}`，不要手写或猜测书页链接
