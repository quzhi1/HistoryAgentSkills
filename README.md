...
一组跑在 Claude Code 上的中国历史 skills。主 skill 基于《中国历史大辞典》和 [cnkgraph](https://cnkgraph.com) 古籍知识图谱 API，回答问题时**必查两边、必引出处（书名 + 章节名）、必说明被引史料、查不到就说查不到、绝不编造**。另有 `random-history-anecdote` 轻量入口，可随机抽取一个有趣历史小段子，按译文、出处、出处考证、职官表、原文、meme 推荐的顺序输出（出处考证里辞典解释与我的评价分开写）；段子里出现官职/爵位/官阶时，才有“职官表”一节；末尾给几个 imgflip 做图链接和每格文案，不附样图。
...
史料原文、译文、证据作用必须分开：**译文只翻译原文**，不得混入“这说明……”或“如果按……来评”等分析。译文还必须**像当代人说的中文**：文言虚词和生僻单字词换成日常说法，该拆句补主语就拆就补，不留半文半白的腔调（专名、官名、年号、书名照旧保留）。原文里的**诗、词、歌、赋等韵文照录原文，不译成白话**，只翻周围的散文。「姓＋官名」式代称（「程光禄」「李司空」「王右军」）要先查出本名，译文里写本名，查不到就注明未考出。历史评价/比较题中，**每个评价或比较结论**都要回扣已查询的辞典或原文证据；像“宋也可以争”这类候选，如果没有原文短引、识典检索链接和被引史料说明，就不能写成结论。
...
这个入口会先运行 `scripts/random_anecdote_seed.py` 动态发现候选：脚本随机生成古汉语检索探针，调用 cnkgraph `Book/Find` 搜全库并随机跳页，不读取固定段子池。之后再用 cnkgraph 查原文、用《中国历史大辞典》核定人物或书名，并**单独查出处那本书**评估可信度。输出刻意很短，顺序是译文、出处、出处考证、职官表、原文、meme 推荐：译文直接给正文、不写“译文：”标签，也不另写“好玩处”，并且要用当代汉语说人话、不留文言腔；原文里的诗词韵文照录不译，「姓＋官名」代称还原成本名。出处考证一行把“辞典解释”（辞典对该书的定性）和“我的评价”（自己的可信度判断）分开写，辞典未收该书时如实说明。段子里若出现官职、爵位、官阶，会再查辞典、结合段子朝代加一节“职官表”解释头衔（辞典未收则明说）；没有这类头衔就不写。这里“短”指不展开史论，原文 1000 字以内都可接受，优先保留完整情节。最后一节“meme 推荐”按段子的**结构**（误解、反转、冷热对比、计划翻车等）从 SKILL 里的固定 imgflip 模板表挑 2–3 个，只给 `https://imgflip.com/memegenerator/<slug>` 做图链接和每格文案，**不附样图、不附图片链接、不自己画图**，也不临时编 slug。它沿用主 workflow 的年号换算和古地名今地括注，但明确**不附识典古籍链接**、**不附左图右史地图链接**；查不到可靠原文就重跑发现或明说。
...
├── random-history-anecdote/
│   └── SKILL.md                    # 随机历史小段子 skill：短答、随机、无识典/地图链接、末尾配 meme
├── README.md                       # 本文件
...
2. **史料引用必须包含书名 + 章节名**：✅《魏书》卷三五《崔浩传》｜❌《魏书》记载｜❌ 据史书记载
   - **史料 ≠ 史书**：正史之外，诸子（《墨子》《庄子》《吕氏春秋》《淮南子》等）、类书、方志、笔记、野史、小说都是合法史料；制度/风俗/名物/乐舞/思想/传说类主题，步骤4.1 必跑「文献部类/体裁」角度跨经·史·子·集检索（主题有别名/上位词时换词再扫，如「桑林之舞」↔「桑林」），不能只往正史里找。证据权重可分层，但权重低不等于可以漏掉——只见于诸子/野史的关键记载必须照引并说明可信度限度
...
- ✅ 同名异地防护与通行地理常识兜底：`place_resolver.py` 返回 `resolved` 落点若与史料语境明显冲突（如西北战事中的“西安”“兰州”落到浙江、云南同名地），按同名异地处理、不采用；TGAZ、辞典、cnkgraph 三来源都无今地线索时默认“今地未能确认”，仅可补一条紧跟“（通行地理常识，非本系统辞典/TGAZ 核验结果）”标注的今地作定位参考，且不得用作 `--admin`、左图右史链接或任何证据
